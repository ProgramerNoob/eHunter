...
7. **Structure Reference**: Use the `TEMPLATE_CONTENT` resolved in Setup for the title, meta section, category headings, ownership note, notes section, and ID formatting. Preserve the resolved override/preset choices throughout generation. If resolution fails or returns empty content, stop and report the template error; do not substitute a hard-coded core template or fallback structure.
