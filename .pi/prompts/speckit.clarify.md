...
5. Sequential questioning loop (interactive):
    ...
    - If no valid questions exist at start, skip questioning and integration. Leave the spec and checklist unchanged, then complete Mandatory Post-Execution Hooks and report that no critical ambiguities were found.
...
7. Validation (for a changed spec, performed after EACH write plus a final pass):
...
8. **Finalize the spec**: If accepted answers changed the spec this session, ensure all validated updates are saved to `FEATURE_SPEC` (step 6 already saves each integration). Write only pending content changes. If no answer was accepted, preserve the original spec and continue to the post-execution hooks and completion report.

9. **Re-validate Spec Quality Checklist** (only when the spec changed this session and the checklist exists):
   - If the spec is unchanged, leave the checklist untouched and report that re-validation was not needed.
   - Otherwise, check if `FEATURE_DIR/checklists/requirements.md` exists. If absent, report that there was no checklist to re-validate.
   - If the spec changed and the checklist exists:
     ...
     4. Re-evaluate each checkbox item against the **updated** spec saved by steps 6 and 8.
...
- Spec path and whether it was updated or left unchanged.
- Sections touched (list names, or "none" if unchanged).
...
- [ ] All accepted answers integrated into the spec; if no answers were accepted, report why the spec is unchanged (no meaningful ambiguity, only low-impact questions, or early termination)
- [ ] Coverage reported, with remaining ambiguities classified as Outstanding or Deferred; do not invent questions to satisfy completion
- [ ] Spec quality checklist re-validated against the updated spec when the spec changed and `FEATURE_DIR/checklists/requirements.md` exists; otherwise report that re-validation was not needed
