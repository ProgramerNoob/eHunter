...
> 迁移来源：`specs/001-add-pageflip-toggle/research.md`（基线 `647ff26`）。正文保全，路径已重定位；命令与源码路径均从仓库根目录解释。
...
**历史记录与当前决策**：以下保留 2026-02-18 研究原文。决策 4 中无效值直接回退拟真、决策 5 中“未显式覆盖时遵循系统”的含糊范围，已由 [2026-09-18 决策](../../decisions/2026-09-18-reader-behavior.md) 第 2、3 节替代：先逐项迁移，无合法值才按系统优先、设备默认初始化并保存；不实时跟随系统。全局范围为同浏览器同脚本安装 GM 共享，降级按 origin 隔离。当前模型与 quickstart 已同步，实施验收见 tasks 后续任务。
