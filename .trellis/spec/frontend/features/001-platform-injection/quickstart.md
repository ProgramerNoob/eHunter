...
> 迁移来源：`specs/001-platform-injection/quickstart.md`（基线 `647ff26`）。正文保全，路径已重定位；命令与源码路径均从仓库根目录解释。
...
> 本文保留新增平台教学示例与旧核对清单；实际启动链以[目录指南](../../directory-structure.md)为准（service factory 位于 `src/platform/factory.ts`）。EH 从 `/s/` 图片页按 [eh-test](../../../../../.pi/skills/eh-test/SKILL.md) 使用生产包验收；本地 TEST 才使用 Vite。旧 `/g/` 直达和 60 秒检查不作为当前通过标准。
<!-- migration-note:end -->
...
2. **Test each platform** using [ego-browser](../../browser-acceptance.md) (follow the linked guide for common operations):
