...
> 迁移来源：`specs/002-more-settings-modal/research.md`（基线 `647ff26`）。正文保全，路径已重定位；命令与源码路径均从仓库根目录解释。
...
**历史记录与当前决策**：以下保留 2026-02-18 研究原文。Decision 5 的存储/非法值回退描述已由 [2026-09-18 决策](../../decisions/2026-09-18-reader-behavior.md) 第 2–4 节细化：GM 同安装共享，不可用时按 origin 降级；合法共享、本站统一旧值、独立旧值、默认值逐项补缺，幂等且重置防复活；动效仅初始化时计算，放大镜尺寸倍率长期保存、开关仅当前页面会话。Decision 4 的二次确认要求继续有效。当前模型与 quickstart 已同步，实施验收见 tasks 后续任务。
