...
> 迁移来源：`specs/002-more-settings-modal/spec.md`（基线 `647ff26`）。正文保全，路径已重定位；命令与源码路径均从仓库根目录解释。
...
**Feature Branch（历史分支）**: `001-more-settings-modal`\
**Created**: 2026-02-18\
**Status**: Draft\
...
**决策依据（2026-09-18）**：正文已同步 [已确认决策](../../decisions/2026-09-18-reader-behavior.md) 第 2–4 节；共享、迁移与相关默认值按该记录执行。模型、契约及 quickstart 已同步，实现与新规则验收另见 [tasks.md](tasks.md)。原始 Input 和历史记录保留演进背景。
...
- **FR-014**: 系统 MUST 持久化配置与快捷项排序并在后续会话恢复；同浏览器同脚本安装内 EH/EX/NH 通过 GM 共享，GM 不可用时按 origin 降级到 localStorage，不包含跨设备同步。快捷项排序使用全局唯一顺序。逐项迁移须保留合法共享值，再按本站合法统一旧设置、独立旧设置补缺，最后才用默认值；共享为空时首个站点先迁入，后续站点只补缺。保留旧副本供降级，GM 恢复后合法共享优先；重复迁移结果一致。
...
- **FR-018**: 系统 MUST 在用户确认执行“清空缓存”后同时重置全部用户配置（含统一设置与快捷配置偏好）为默认值，并阻止保留的旧副本再次导入已重置的偏好。翻页默认值按 FR-021 初始化。
- **FR-019**: 系统 MUST 在顶部快捷配置栏渲染时按“全局排序 + 当前模式过滤”规则展示配置项。
- **FR-020**: 系统 MUST 在执行“清空缓存并重置全部设置”前提供二次确认，并清晰提示该操作将恢复默认配置且不可撤销。
- **FR-021**: 翻页动效仅在无合法迁移值时初始化：系统减少动态效果开启则无动效，否则桌面拟真、移动平移。保存初始化结果；后续系统变化、保存其他设置不重算。已有合法旧值即使等于旧默认也保留，用户选择可以覆盖。
- **FR-022**: 放大镜沿用桌面端限制，尺寸为 20–300px 任意整数、默认 80px，倍率为 2/3/4/5、默认 3；二者长期保存并遵循 FR-014。快捷菜单与设置范围一致、尺寸每次增减 10px，跨界停在边界且边界按钮不可继续操作。开关仅当前页面会话继承，刷新或新页面关闭；受限空间的临时取样尺寸不得写回偏好，几何规则见放大镜规格。

FR-021、FR-022 的验收入口见 [quickstart 第 4 节](quickstart.md#4-相关设置专项)，包含首次初始化、保持规则、尺寸边界与记忆范围。
