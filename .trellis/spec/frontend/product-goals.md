...
> 迁移来源：`design.md`（基线 `647ff26`）。正文保全，路径已重定位；命令与源码路径均从仓库根目录解释。
<!-- migration-note:end -->
...
## 已确认决策
本文描述产品目标，不代表当前实现已全部完成。涉及下载格式、翻页默认值、偏好存储/迁移和放大镜时，先读取 [2026-09-18 决策记录](decisions/2026-09-18-reader-behavior.md)，按其中已确认规则修改和验收；记录同时列出待同步规格与待修正实现。
...
- 书页模式翻页动效支持用户下拉配置：
  ...
  - 平移切屏
  - 无动效直接切页
- 翻页初始化、已存偏好优先级与系统偏好生效时机遵循 [已确认决策](decisions/2026-09-18-reader-behavior.md)。

## 功能说明

### 基础功能
- 多网站信息解析（EH 优先，NH 次之）
- 通用偏好持久化（作用域、介质与迁移规则见下文“配置与持久化”）
...
### 配置存储
- 共享范围、GM 优先及本站存储降级、旧偏好补缺顺序以 [已确认决策](decisions/2026-09-18-reader-behavior.md) 为准。
